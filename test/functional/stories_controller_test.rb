...
class StoriesControllerTest < ActionController::TestCase
  ...
  def test_should_reject_missing_story_attribute_name
    post_with_user :create, :story => {:link => 'http://www.test.com/'}
    assert assigns(:story).errors.on(:name)
  end
  
  def test_should_reject_missing_story_attribute_link
    post_with_user :create, :story => {:name => 'test story'}
    story = assigns(:story)
  ...
  def test_should_show_navigation_menu
    get :index
    assert_select 'ul#navigation li', :count => 3
  end
  ...
  def test_should_only_list_unpromoted_in_bin
    get :bin
    assert_equal [stories(:one), stories(:two)], assigns(:stories)
  end
